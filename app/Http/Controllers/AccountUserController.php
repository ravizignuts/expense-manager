...
class AccountUserController extends Controller
{
    ...
    public function delete($id)
    {
        try{
            ...
            try{
                $account_user = AccountUser::withTrashed()->findOrFail($id);
                $account_user->forceDelete();
                return response()->json(['message' => 'Account User Deleted Forcefully']);
            }catch(ModelNotFoundException $e){
                return response()->json(['message' => 'Account User Not Found']);
            }
        }
    }
    /**
     * API For Restore Account User
     * @param $id
     * @return Json data
     */
    ...
    {
        $account_user = AccountUser::withTrashed()->findOrFail($id);
        $account_user->restore();
        return $this->restoreResponse('Account User', $account_user);
    }
    /**
     * API For list User In Account
     * @param $id
